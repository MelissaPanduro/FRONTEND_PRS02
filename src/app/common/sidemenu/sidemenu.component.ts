...
export class SidemenuComponent implements OnInit, OnDestroy {
  ...
  // Utilizamos HostListener para detectar cambios de tamaño de ventana
  ...
  private checkScreenSize(): void {
    if (this.isBrowser) {
      ...
      if (window.innerWidth >= 1024) {
        ...
        // Solo cerramos en móvil si ya estaba cerrado
        ...
        item.children.forEach((child, childIndex) => {
          ...
          if (child.children) {
            child.children.forEach((grandChild, grandChildIndex) => {
              ...
              // Revisamos un nivel más profundo si es necesario
              if (grandChild.children) {
                grandChild.children.forEach((greatGrandChild) => {
                  if (this.checkPathMatch(greatGrandChild, currentUrl)) {
                    this.dropdownIndex = index;
                    this.subDropdownIndex.set(index, childIndex);
  ...
  logout(): void {
    ...
    // En móvil, cerramos el sidebar al abrir el modal
    if (this.isMobile) {
      this.closeSidebar();
    }
  }
  ...
  confirmLogout(): void {
    // Mostrar indicador de carga o desactivar botones si es necesario
